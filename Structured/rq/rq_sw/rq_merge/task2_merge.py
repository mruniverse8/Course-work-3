...
def evaluate(args, model, tokenizer,file_name, range_windows,eval_when_training=False):
    ...
    code_vecs = np.concatenate((code_vecs, code_vecs2), axis=0)
    newcode_dataset.examples = code_dataset.examples + newcode_dataset.examples
    #we need to merge the datasets also in newcode_dataset!!!
